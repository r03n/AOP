...
public class AdminUI {
    ...
    private JPanel createAdminEventsPanel(Admin a, MainGUI app) {
        ...
        table.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mouseClicked(java.awt.event.MouseEvent e) {
                if (e.getClickCount() == 2) { 
                    ...
                    if (row != -1) openEventReport(app, a, (int)table.getValueAt(row, 0), (String)table.getValueAt(row, 1), (String)table.getValueAt(row, 3), (String)table.getValueAt(row, 4), (int)table.getValueAt(row, 5));
                }
            }
        });
        ...
        createBtn.addActionListener(e -> {
            ...
            while (true) {
                if (JOptionPane.showConfirmDialog(app.getFrame(), msg, "Create Event", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
                    ...
                    } catch (NumberFormatException ex) { JOptionPane.showMessageDialog(app.getFrame(), "Capacity must be a valid number.", "Error", JOptionPane.ERROR_MESSAGE); }
                } else break;
            }
        });
        ...
        editBtn.addActionListener(e -> {
            int row = table.getSelectedRow();
            if (row == -1) { JOptionPane.showMessageDialog(app.getFrame(), "Select an event to edit."); return; }
            ...
            while (true) {
                if (JOptionPane.showConfirmDialog(app.getFrame(), msg, "Edit Event", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
                    if (titleF.getText().trim().isEmpty() || capF.getText().trim().isEmpty()) {
                        JOptionPane.showMessageDialog(app.getFrame(), "Please fill in all required fields.", "Error", JOptionPane.ERROR_MESSAGE);
                        continue;
                    }
        ...
        delEvtBtn.addActionListener(e -> {
            int row = table.getSelectedRow();
            if (row != -1 && JOptionPane.showConfirmDialog(app.getFrame(), "Terminate event?", "Confirm", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                JOptionPane.showMessageDialog(app.getFrame(), a.deleteAnyEvent((int) table.getValueAt(row, 0))); refreshTable.run();
            }
        });
    ...
    private JPanel createAdminUsersPanel(Admin a, MainGUI app) {
        ...
        createBtn.addActionListener(e -> {
            ...
            while(true) {
                if (JOptionPane.showConfirmDialog(app.getFrame(), msg, "Create Participant", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
        ...
        promoteBtn.addActionListener(e -> {
            ...
            if(r != -1) { JOptionPane.showMessageDialog(app.getFrame(), a.changeUserRole((String)table.getValueAt(r, 1), "Organizer")); refreshTable.run(); }
        });

        demoteBtn.addActionListener(e -> {
            int r = table.getSelectedRow();
            if(r != -1) { JOptionPane.showMessageDialog(app.getFrame(), a.changeUserRole((String)table.getValueAt(r, 1), "Participant")); refreshTable.run(); }
        });

        purgeBtn.addActionListener(e -> {
            int r = table.getSelectedRow();
            if (r != -1 && JOptionPane.showConfirmDialog(app.getFrame(), "Purge this account permanently?", "Confirm", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                JOptionPane.showMessageDialog(app.getFrame(), a.deleteAccount((String)table.getValueAt(r, 1))); refreshTable.run();
            }
        });
    ...
    private void openEventReport(MainGUI app, Admin a, int eventId, String title, String date, String time, int capacity) {
        JDialog dialog = new JDialog(app.getFrame(), "Admin Management: " + title, true);
        dialog.setSize(450, 400); dialog.setLocationRelativeTo(app.getFrame()); dialog.setLayout(new BorderLayout(10, 10));
